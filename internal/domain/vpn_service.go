...
func withSetupHint(err error) error {
	...
	case errors.Is(err, ErrConfigMissing):
		return fmt.Errorf("%w\n\nОткройте «Настройки» в меню трея и вставьте свой config.json в разделе «Первоначальная настройка». Конфиг с параметрами, которые приложение не принимает (пути к файлам, прослушивание не на 127.0.0.1), кладут по этому пути вручную: папка данных установленной копии доступна только администраторам.", err)
	}
	return err
}
