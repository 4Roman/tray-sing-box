...
type winTray struct {
	...
	// PATCH(tray-sing-box): the icon is not in the notification area (yet),
	// and whether the retry goroutine is running. Guarded by muNID.
	iconPending  bool
	iconRetrying bool
...
// PATCH(tray-sing-box): ensureIcon adds the icon to the notification area and,
// when that fails, keeps retrying in the background. Upstream called NIM_ADD
// exactly once: a failure during initInstance (no taskbar yet, or explorer too
// busy at logon — Shell_NotifyIcon times out) aborted the whole initialization
// before the menu was created, onReady was never called, and the application
// stayed headless for good. t.nid accumulates icon and tooltip while pending
// (setIcon/setTooltip update it before NIM_MODIFY), so a late NIM_ADD shows
// the complete icon.
func (t *winTray) ensureIcon() {
	t.muNID.Lock()
	err := t.addOrConfirmIcon()
	t.iconPending = err != nil
	startRetry := t.iconPending && !t.iconRetrying
	if startRetry {
		t.iconRetrying = true
	}
	t.muNID.Unlock()
	...
	if err != nil {
		log.Errorf("Unable to add the tray icon, will keep retrying: %v", err)
	}
	if startRetry {
		go t.retryIcon()
	}
...
func (t *winTray) retryIcon() {
	for {
		time.Sleep(2 * time.Second)

		t.muNID.Lock()
		if t.nid == nil || !t.iconPending {
			t.iconRetrying = false
			t.muNID.Unlock()
			return
		}
		if err := t.addOrConfirmIcon(); err == nil {
			t.iconPending = false
			t.iconRetrying = false
			t.muNID.Unlock()
			log.Debugf("Tray icon added after retrying")
			return
		}
		t.muNID.Unlock()
	}
...
// addOrConfirmIcon adds the icon; NIM_ADD also fails when the icon is already
// there (a TaskbarCreated broadcast that did not wipe it), which NIM_MODIFY
// tells apart from "no taskbar". Must be called with muNID held.
func (t *winTray) addOrConfirmIcon() error {
	err := t.nid.add()
	if err != nil && t.nid.modify() == nil {
		return nil
	}
	return err
}
...
// PATCH(tray-sing-box): takes the icon handle (built from memory, see
// iconFromBytes) instead of a file path
func (t *winTray) setIcon(h windows.Handle) error {
...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case WM_CLOSE:
		...
		t.wcex.unregister()
	case WM_DESTROY, WM_ENDSESSION:
		// PATCH(tray-sing-box): WM_ENDSESSION with wParam == FALSE means the
		// shutdown/logoff was CANCELLED — the session goes on. Upstream deleted
		// the icon and ran onExit regardless, leaving a running app without a
		// tray icon. (WM_DESTROY used to fall through into the same branch;
		// its wParam is always 0, hence the explicit message check.)
		...
		t.muNID.Lock()
		if t.nid != nil {
			t.iconPending = false // stops the retry goroutine
			t.nid.delete()
		}
		t.muNID.Unlock()
		systrayExit()
	...
	case t.wmTaskbarCreated: // on explorer.exe restarts
		// PATCH(tray-sing-box): upstream ignored a failed re-add
		t.ensureIcon()
	default:
...
func (t *winTray) initInstance() error {
	...
	// PATCH(tray-sing-box): a failed NIM_ADD is not fatal any more, see
	// ensureIcon — the menu is still created and onReady still runs
	t.ensureIcon()
	return nil
}
...
// PATCH(tray-sing-box): icons are built from memory. Upstream wrote the icon
// bytes to %TEMP%\systray_temp_icon_<md5> and loaded that file: in an
// elevated app the user's TEMP (and the predictable name in it) is under the
// control of the user's non-elevated programs — a pre-created file fed the
// elevated process foreign image bytes, and a redirected TEMP turned the
// write into a file creation at a chosen path.
func (t *winTray) iconFromBytes(iconBytes []byte) (windows.Handle, error) {
	...
	cx, _, _ := pGetSystemMetrics.Call(SM_CXSMICON)
	cy, _, _ := pGetSystemMetrics.Call(SM_CYSMICON)
	offset, size, err := pickIconImage(iconBytes, int(cx))
	if err != nil {
		return 0, err
	}
	res, _, callErr := pCreateIconFromResourceEx.Call(
		uintptr(unsafe.Pointer(&iconBytes[offset])),
		uintptr(size),
...
// pickIconImage finds, in the bytes of an .ico file, the image to build a
// size x size icon from: that exact width, else the smallest larger one,
// else the largest. Returns its offset and length.
func pickIconImage(ico []byte, size int) (int, int, error) {
	if len(ico) < 6 || binary.LittleEndian.Uint16(ico[0:]) != 0 || binary.LittleEndian.Uint16(ico[2:]) != 1 {
		return 0, 0, fmt.Errorf("not an .ico file")
	}
	...
	better := func(w int) bool {
		...
		case w == size:
			return true
		case w > size && (bestWidth < size || w < bestWidth):
			return true
		case w < size && bestWidth < size && w > bestWidth:
			return true
		}
	...
	for i := 0; i < count; i++ {
		e := 6 + 16*i
		if e+16 > len(ico) {
			return 0, 0, fmt.Errorf("truncated .ico directory")
		}
		w := int(ico[e])
		if w == 0 {
			w = 256
		}
		length := int(binary.LittleEndian.Uint32(ico[e+8:]))
		offset := int(binary.LittleEndian.Uint32(ico[e+12:]))
	...
	if best < 0 {
		return 0, 0, fmt.Errorf("no usable image in the .ico file")
	}
	e := 6 + 16*best
	return int(binary.LittleEndian.Uint32(ico[e+12:])), int(binary.LittleEndian.Uint32(ico[e+8:])), nil
}
...
func SetIcon(iconBytes []byte) {
	...
	if err != nil {
		log.Errorf("Unable to build the icon: %v", err)
		return
	}
	if err := wt.setIcon(h); err != nil {
